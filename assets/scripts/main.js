...
function initializeServiceWorker() {
  ...
  if (!("serviceWorker" in navigator)) {
    console.log("Service workers not supported on this browser")
    return;
  }
  ...
  return new Promise((resolve, reject) => {
    for(let i = 0; i < RECIPE_URLS.length; i++){
      try{
        ...
        .then((json) => {
            recToReturn.push(json);
            console.log(json);
            if(recToReturn.length == RECIPE_URLS.length){
              saveRecipesToStorage(recToReturn);
              resolve(recToReturn);
            }
        });
      }
